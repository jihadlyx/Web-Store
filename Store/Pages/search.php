        ...
        <div class="container">
            <?php 
                if(isset($_POST["search"])){
                    $sql = "SELECT products.id, products.name, price, company, quantity, details, path, phone, type_product.name AS type FROM `products` JOIN employees ON employees.id = id_employee JOIN type_product ON type_product.id = products.type_id WHERE products.name LIKE '%". $_POST["search"] ."%'";
                    $query = $conn->prepare($sql);
                    $query->execute();
                    $result = $query->fetchALL(PDO::FETCH_OBJ);
            ?>
            <h2 class="main-text" style="margin-top: 60px"> <?= "بحث عن '". $_POST["search"] ."'"?></h2>
            <p>
            </p>
